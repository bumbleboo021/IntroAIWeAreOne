...
# Đặt cấu hình RTC (có thể không cần thiết trong môi trường local)
# RTC_CONFIGURATION = RTCConfiguration(
#     {"iceServers": [{"urls": ["stun:stun.l.google.com:19302"]}]}
# )

# --- THAY THẾ HOẶC THÊM VÀO TRONG KHỐI if not st.session_state['analysis_done']: ---

# Khối Ghi âm trực tiếp
st.markdown("---")
st.subheader("🎤 Ghi âm Trực tiếp")
st.info("Nhấn 'Start' để bắt đầu ghi âm qua Microphone.")

# LƯU Ý: Đây là ví dụ đơn giản để kích hoạt Mic. 
# Để có tính năng 'Ghi' và 'Lưu file', bạn cần sử dụng AudioProcessorBase (một kỹ thuật phức tạp hơn).

try:
    # Kích hoạt luồng WebRTC chỉ với Audio
    webrtc_ctx = webrtc_streamer(
        key="local_audio_stream",
        mode=WebRtcMode.SENDONLY, # Chỉ gửi dữ liệu từ client (mic) lên server
        rtc_configuration=None, # Dùng cấu hình mặc định
        media_stream_constraints={
            "video": False,
            "audio": True,
        },
        # Hiển thị nút "Start" và "Stop"
    ...
    if webrtc_ctx.state.playing:
        st.warning("Đang ghi âm trực tiếp... (Dữ liệu âm thanh đang được truyền).")
        st.text("Để tích hợp với Gemini, bạn cần thêm AudioProcessor để thu thập và đóng gói dữ liệu khi dừng.")
    
except Exception as e:
    st.error(f"Lỗi khởi tạo ghi âm trực tiếp: {e}")

st.markdown("---")
# ... (Phần file_uploader và kích hoạt phân tích giữ nguyên)
with st.sidebar:
    st.markdown("""
